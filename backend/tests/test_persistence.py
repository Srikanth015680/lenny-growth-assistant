...
async def test_message_persists_sources(db_session):
...
async def test_artifact_links_to_message(db_session):
...
async def test_deleting_session_cascades_to_messages_and_artifacts(
    db_session,
):
    ...
    artifacts = (
        await db_session.execute(select(Artifact))
    ).scalars().all()
